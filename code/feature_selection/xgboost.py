...
def objective(trial, x_train, y_train, x_val, y_val):
    ...
    xgb_model = xgb.XGBRegressor(device='cuda', missing=np.nan, random_state=42, n_estimators=n_trees, booster=boosting_type, max_depth=max_depth, learning_rate=learning_rate, reg_alpha=l1_norm, reg_lambda=l2_norm, sampling_method="uniform", subsample=subsample)
...
for fold in range(0, 5):
    ...
    study.optimize(lambda trial: objective(trial, train_input_data, train_y[fold].copy(), val_relevant_input, validation_y[fold].copy()), n_trials=1000)
